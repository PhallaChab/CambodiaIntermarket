...
class Products {
	public static function insert($pname,$pprice,$pcode,$pcat,$pimage,$deskh,$desen,$pinfor,$date){
        $sql = "INSERT INTO products (pro_name,pro_price,pro_code,cat_id,pro_image,pro_descriptionKh,pro_descriptionEn,pro_information,create_date) values ('{$pname}','{$pprice}','{$pcode}','{$pcat}','{$pimage}','{$deskh}','{$desen}','{$pinfor}','{$date}')";
        return runNonQuery($sql);
    }
   
    public static function edit($id,$pname,$pprice,$pcode,$pcat,$pimage,$deskh,$desen,$pinfor){
    	$sql = "UPDATE products SET pro_name='$pname',pro_price='$pprice',pro_code='$pcode',cat_id='$pcat',pro_image='$pimage',pro_descriptionKh='$deskh',pro_descriptionEn='$desen',pro_information='$pinfor' where pro_id='$id'";
    	return runNonQuery($sql);
    }
